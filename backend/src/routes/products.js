    ...
    for (const s of scraped) {
      try {
        const [result] = await pool.execute(
          ...
          [ productId, s.source, s.external_id, s.reviewer_name, s.rating, s.title, s.content, s.review_date ]
        );
        if (result.affectedRows === 1) added++; else skipped++;
      } catch (err) {
        if (String(err.code) === 'ER_DUP_ENTRY') skipped++; else throw err;
      }
    }
...
router.get('/:id/aggregate', async (req, res, next) => {
  try {
    ...
    histRows.forEach(r => { hist[String(r.rating)] = r.c; });
